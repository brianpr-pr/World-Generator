...
2. Establishment of amount of squares for each zone, we first validate that the product of the factors:
    zones generated and  maximum size of zones is samller or equal than the maximum total size of all zones of a section, in case this product is bigger with the help of the ternary operator we either assign 1 or half rounded to a whole number and
    we check again if the new number is able to pass the condition, in case the product is smaller or equal we generate a random number that is in the range of 1-Maximum-Size-Zone.
